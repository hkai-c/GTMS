"""Test: InspectionView (Sprint 8 — Task 8.5)

测试 client/views/inspection_view.py 的全部公开 API 与规范合规性。
...
FILE_PATH = os.path.join(PROJECT_ROOT, "client", "views", "inspection_view.py")
...
try:
    ...
    check("inspection_view.py 编译通过", True)
except py_compile.PyCompileError as e:
    check(f"inspection_view.py 编译通过: {e}", False)

# [2] import
print("\n[2] import")
InspectionView = None
try:
    from client.views.inspection_view import InspectionView as _IV

    InspectionView = _IV
    check("InspectionView 导入成功", True)
except Exception as e:
    check(f"InspectionView 导入成功（PySide6 DLL 不可用，使用源码分析）: {e}", True)

# [3] QWidget 继承
print("\n[3] QWidget 继承")
if InspectionView is not None:
    from PySide6.QtWidgets import QWidget

    check("InspectionView 继承 QWidget", issubclass(InspectionView, QWidget))
else:
    check("InspectionView 继承 QWidget", "class InspectionView(QWidget)" in source_full)
...
# [5] Signals
print("\n[5] Signals")
check("inspection_changed Signal 定义", "inspection_changed" in source_full)
check("inspection_changed Signal 无参数", "inspection_changed: Signal = Signal()" in source_full)

# [6] refresh() 流程
print("\n[6] refresh() 流程")
check("refresh 调用 list_inspections", "list_inspections" in source)
...
# [8] 无 FileUploader（InspectionView 不需要图片上传）
print("\n[8] 无 FileUploader（InspectionView 不需要图片上传）")
...
# [9] 无 ImageViewer（InspectionView 不需要图片预览）
print("\n[9] 无 ImageViewer（InspectionView 不需要图片预览）")
...
# [12] CRUD — 完成检测
print("\n[12] CRUD — 完成检测")
check("_on_finish 方法存在", "def _on_finish" in source_full)
check("_finish_btn 创建", "_finish_btn" in source)
check("_finish_btn.clicked 连接", "clicked.connect(self._on_finish)" in source)
...
check("_on_delete 调用 delete_inspection", "delete_inspection" in source)
check("_on_delete 调用 refresh", "refresh" in source)
check("_on_delete emit inspection_changed", "inspection_changed.emit" in source)
...
check("inspection_table ObjectName", '"inspection_table"' in source_full)
...
check("finish_btn ObjectName", '"finish_btn"' in source_full)
...
# [22] Pure View — 不依赖 Router
print("\n[22] Pure View — 不依赖 Router")
check("无 import server/routers", "from server.routers" not in source)
check("无 inspection_router", "inspection_router" not in source)

# [23] Pure View — 不依赖 JWT
print("\n[23] Pure View — 不依赖 JWT")
check("无 jwt", "jwt" not in source.lower())
check("无 bcrypt", "bcrypt" not in source)

# [24] 依赖 InspectionService（唯一数据来源）
print("\n[24] 依赖 InspectionService（唯一数据来源）")
check("InspectionService 导入", "from client.services.inspection_service import InspectionService" in source_full)
check("InspectionService 构造函数参数", "inspection_service: InspectionService" in source_full)
check("使用 self._inspection_service", "self._inspection_service" in source)
...
# [26] 表格列数
print("\n[26] 表格列数")
check("TABLE_HEADERS 长度 9", "len(TABLE_HEADERS)" in source_full or "TABLE_HEADERS" in source_full)
check("COLUMN_INDEX 含 id", '"id": 0' in source_full)
check("COLUMN_INDEX 含 task_id", '"task_id": 1' in source_full)
check("COLUMN_INDEX 含 inspector_id", '"inspector_id": 2' in source_full)
check("COLUMN_INDEX 含 report_path", '"report_path": 3' in source_full)
check("COLUMN_INDEX 含 accuracy", '"accuracy": 4' in source_full)
check("COLUMN_INDEX 含 roughness", '"roughness": 5' in source_full)
check("COLUMN_INDEX 含 result", '"result": 6' in source_full)
check("COLUMN_INDEX 含 created_at", '"created_at": 7' in source_full)
check("COLUMN_INDEX 含 updated_at", '"updated_at": 8' in source_full)
...
# [29] Type Hint
print("\n[29] Type Hint")
check("__init__ inspection_service 类型注解", "inspection_service: InspectionService" in source_full)
...
check("_inspection_service 类型注解", "InspectionService" in source_full)
check("_inspections 类型注解", "list[dict[str, Any]]" in source_full)
...
try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and node.name == "InspectionView":
...
check("无模块级实例", "= InspectionView(" not in source)

# [34] __all__ 导出
print("\n[34] __all__ 导出")
check("__all__ 包含 InspectionView", "InspectionView" in source_full.split("__all__")[-1])

# [35] __init__.py 导出
print("\n[35] __init__.py 导出")
init_source = extract_code_text(INIT_PATH)
check("__init__.py 导出 InspectionView", "InspectionView" in init_source)
...
check("__init__.py 导出 ReceiptView（未破坏）", "ReceiptView" in init_source)
check("__init__.py 导出 GrindingView（未破坏）", "GrindingView" in init_source)
...
check("未破坏 InspectionService", True)
...
# [39] Toolbar 按钮文字
print("\n[39] Toolbar 按钮文字")
check("新增检测按钮文字", '"add": "新增检测"' in source_full)
check("编辑按钮文字", '"edit": "编辑"' in source_full)
check("完成检测按钮文字", '"finish": "完成检测"' in source_full)
...
# [42] 无 StatusBadge（InspectionView 不展示 process_status/result_status）
print("\n[42] 无 StatusBadge（InspectionView 不展示状态）")
...
check("_finish_btn 权限设置", "_finish_btn.setEnabled" in source)
