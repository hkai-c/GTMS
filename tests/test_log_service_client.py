"""Test: Desktop LogService (Sprint 11 — Task 11.5)

严格依据 DEVELOPMENT_ROADMAP.md Task 11.5 验收标准。
测试 client/services/log_service.py 全部公开 API 与代码规范。
...
print("=" * 60)
print("  Task 11.5 — Desktop LogService Self Test")
print("=" * 60)

SOURCE_PATH = os.path.join("client", "services", "log_service.py")
...
check("LogService 有 __init__", hasattr(LogService, "__init__"))
...
# ----------------------------------------------------------
print("\n[7] HTTP Mapping")
check("list_logs -> GET /api/log",
      "/api/log" in source
      and "self._api_client.get" in source)
check("get_log -> GET /api/log/{log_id}",
      "/api/log/" in source)
check("create_log -> POST /api/log",
      "/api/log" in source
      and "self._api_client.post" in source)
check("export_logs -> POST /api/log/export",
      "/api/log/export" in source)

# ----------------------------------------------------------
# [8] 返回 resp.json()
# ----------------------------------------------------------
print("\n[8] 返回 resp.json()")
check("list_logs 返回 resp.json()",
      "resp.json()" in code)
check("get_log 返回 resp.json()",
      "resp.json()" in code)
check("create_log 返回 resp.json()",
      "resp.json()" in code)
check("export_logs 返回 resp.json()",
      "resp.json()" in code)
...
# ----------------------------------------------------------
print("\n[12] Query Rules（仅非 None 字段提交）")
check("list_logs nz operator_id",
      "if operator_id is not None:" in source)
check("list_logs nz operation",
      "if operation is not None:" in source)
check("list_logs nz module",
      "if module is not None:" in source)
check("list_logs nz keyword",
      "if keyword is not None:" in source)
check("list_logs nz start_time",
      "if start_time is not None:" in source)
check("list_logs nz end_time",
      "if end_time is not None:" in source)
check("list_logs default page=1",
      "page: int = 1" in source)
check("list_logs default page_size=20",
      "page_size: int = 20" in source)

# ----------------------------------------------------------
# [13] Body Rules（create_log）
# ----------------------------------------------------------
print("\n[13] Body Rules（create_log）")
check("create_log nz description",
      "if description is not None:" in source)
check("create_log body operator_id",
      '"operator_id"' in source)
check("create_log body operation",
      '"operation"' in source)

# ----------------------------------------------------------
# [14] Body Rules（export_logs）
# ----------------------------------------------------------
print("\n[14] Body Rules（export_logs）")
check("export_logs nz operator_id",
      "if operator_id is not None:" in source)
check("export_logs nz operation",
      "if operation is not None:" in source)
check("export_logs nz module",
      "if module is not None:" in source)
check("export_logs nz keyword",
      "if keyword is not None:" in source)
check("export_logs nz start_time",
      "if start_time is not None:" in source)
check("export_logs nz end_time",
      "if end_time is not None:" in source)
...
print("\n[17] 循环导入")
check("无循环导入",
      "from client.services.log_service" not in code)

# ----------------------------------------------------------
# [18] 无 TODO / FIXME / pass
# ----------------------------------------------------------
print("\n[18] 无 TODO / FIXME / pass")
...
check("__init__.py 导出 InspectionService (未破坏)",
      "InspectionService" in init_source)
check("__init__.py 导出 GrindingService (未破坏)",
      "GrindingService" in init_source)
check("__init__.py 导出 ReceiptService (未破坏)",
      "ReceiptService" in init_source)
