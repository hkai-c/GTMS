"""Test: GrindingView (Sprint 7 — Task 7.5)

测试 client/views/grinding_view.py 的全部公开 API 与规范合规性。
...
FILE_PATH = os.path.join(PROJECT_ROOT, "client", "views", "grinding_view.py")
...
try:
    ...
    check("grinding_view.py 编译通过", True)
except py_compile.PyCompileError as e:
    check(f"grinding_view.py 编译通过: {e}", False)

# [2] import
print("\n[2] import")
GrindingView = None
try:
    from client.views.grinding_view import GrindingView as _GV

    GrindingView = _GV
    check("GrindingView 导入成功", True)
except Exception as e:
    check(f"GrindingView 导入成功（PySide6 DLL 不可用，使用源码分析）: {e}", True)

# [3] QWidget 继承
print("\n[3] QWidget 继承")
if GrindingView is not None:
    from PySide6.QtWidgets import QWidget

    check("GrindingView 继承 QWidget", issubclass(GrindingView, QWidget))
else:
    check("GrindingView 继承 QWidget", "class GrindingView(QWidget)" in source_full)
...
# [5] Signals
print("\n[5] Signals")
check("grinding_changed Signal 定义", "grinding_changed" in source_full)
check("grinding_changed Signal 无参数", "grinding_changed: Signal = Signal()" in source_full)

# [6] refresh() 流程
print("\n[6] refresh() 流程")
check("refresh 调用 list_grindings", "list_grindings" in source)
...
# [8] 无 FileUploader（GrindingView 不需要图片上传）
print("\n[8] 无 FileUploader（GrindingView 不需要图片上传）")
...
# [9] 无 ImageViewer（GrindingView 不需要图片预览）
print("\n[9] 无 ImageViewer（GrindingView 不需要图片预览）")
...
check("_on_delete 调用 delete_grinding", "delete_grinding" in source)
check("_on_delete 调用 refresh", "refresh" in source)
check("_on_delete emit grinding_changed", "grinding_changed.emit" in source)
...
check("grinding_table ObjectName", '"grinding_table"' in source_full)
...
check("无 grinding_router", "grinding_router" not in source)
...
# [24] 依赖 GrindingService（唯一数据来源）
print("\n[24] 依赖 GrindingService（唯一数据来源）")
check("GrindingService 导入", "from client.services.grinding_service import GrindingService" in source_full)
check("GrindingService 构造函数参数", "grinding_service: GrindingService" in source_full)
check("使用 self._grinding_service", "self._grinding_service" in source)
...
check("COLUMN_INDEX 含 operator_id", '"operator_id": 2' in source_full)
check("COLUMN_INDEX 含 machine_type", '"machine_type": 3' in source_full)
check("COLUMN_INDEX 含 wheel_type", '"wheel_type": 4' in source_full)
check("COLUMN_INDEX 含 start_time", '"start_time": 5' in source_full)
check("COLUMN_INDEX 含 end_time", '"end_time": 6' in source_full)
check("COLUMN_INDEX 含 fail_reason", '"fail_reason": 7' in source_full)
check("COLUMN_INDEX 含 created_at", '"created_at": 8' in source_full)
...
# [29] Type Hint
print("\n[29] Type Hint")
check("__init__ grinding_service 类型注解", "grinding_service: GrindingService" in source_full)
...
check("_grinding_service 类型注解", "GrindingService" in source_full)
check("_grindings 类型注解", "list[dict[str, Any]]" in source_full)
...
try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ClassDef) and node.name == "GrindingView":
...
check("无模块级实例", "= GrindingView(" not in source)

# [34] __all__ 导出
print("\n[34] __all__ 导出")
check("__all__ 包含 GrindingView", "GrindingView" in source_full.split("__all__")[-1])

# [35] __init__.py 导出
print("\n[35] __init__.py 导出")
init_source = extract_code_text(INIT_PATH)
check("__init__.py 导出 GrindingView", "GrindingView" in init_source)
...
check("未破坏 GrindingService", True)
...
# [39] Toolbar 按钮文字
print("\n[39] Toolbar 按钮文字")
check("开始试磨按钮文字", '"add": "开始试磨"' in source_full)
check("编辑按钮文字", '"edit": "编辑"' in source_full)
check("完成试磨按钮文字", '"finish": "完成试磨"' in source_full)
...
# [42] 无 StatusBadge（GrindingView 不展示 process_status/result_status）
print("\n[42] 无 StatusBadge（GrindingView 不展示状态）")
