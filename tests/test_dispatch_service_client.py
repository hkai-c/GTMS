"""Test: Desktop DispatchService (Sprint 9 — Task 9.4)

严格依据 DEVELOPMENT_ROADMAP.md Task 9.4 验收标准。
测试 client/services/dispatch_service.py 全部公开 API 与代码规范。
...
print("=" * 60)
print("  Task 9.4 — Desktop DispatchService Self Test")
print("=" * 60)

SOURCE_PATH = os.path.join("client", "services", "dispatch_service.py")
...
check("DispatchService 有 __init__", hasattr(DispatchService, "__init__"))
...
# ----------------------------------------------------------
print("\n[7] HTTP Mapping")
check("list_dispatches → GET /api/dispatch",
      '/api/dispatch"' in code or "/api/dispatch'" in code)
check("get_dispatch → GET /api/dispatch/{id}",
      '/api/dispatch/{' in code)
check("create_dispatch → POST /api/dispatch",
      'post("/api/dispatch"' in code)
check("update_dispatch → PUT /api/dispatch/{id}",
      'put(' in code)
check("delete_dispatch → DELETE /api/dispatch/{id}",
      'delete(' in code)
...
# ----------------------------------------------------------
print("\n[12] Query Rules（仅非 None 字段提交）")
check("list_dispatches 仅提交非 None direction",
      "if direction is not None:" in source)
...
# ----------------------------------------------------------
print("\n[13] Body Rules（仅提交非 None 字段）")
check("update_dispatch body 为空 dict",
      "body: dict[str, Any] = {}" in source)
check("update_dispatch 仅提交非 None direction",
      "if direction is not None:" in source)
check("update_dispatch 仅提交非 None dispatch_date",
      "if dispatch_date is not None:" in source)
check("update_dispatch 仅提交非 None operator_id",
...
# ----------------------------------------------------------
print("\n[16] 循环导入")
check("无循环导入", "from client.services.dispatch_service" not in code)
...
# ----------------------------------------------------------
print("\n[18] __all__")
check("__all__ 含 DispatchService", "DispatchService" in source.split("__all__")[-1])
...
check("__init__.py 导出 DispatchService", "DispatchService" in init_source)
