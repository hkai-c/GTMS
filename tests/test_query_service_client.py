...
check("no  Router 导入", "from server.routers" not in source)
...
check("list_tasks -> GET /api/query",
      "/api/query" in source and "self._api_client.get" in source)
check("get_statistics -> GET /api/query/statistics",
      "/api/query/statistics" in source)
check("get_customer_ranking -> GET /api/query/ranking/customers",
      "/api/query/ranking/customers" in source)
check("get_machine_ranking -> GET /api/query/ranking/machines",
      "/api/query/ranking/machines" in source)
check("export_excel -> POST /api/query/export",
      "/api/query/export" in source and "self._api_client.post" in source)

# ----------------------------------------------------------
# [8] 返回 resp.json()
# ----------------------------------------------------------
print("\n[8] 返回 resp.json()")
check("list_tasks 返回 resp.json()", "resp.json()" in code)
check("get_statistics 返回 resp.json()", "resp.json()" in code)
check("get_customer_ranking 返回 resp.json()", "resp.json()" in code)
check("get_machine_ranking 返回 resp.json()", "resp.json()" in code)
check("export_excel 返回 resp.json()", "resp.json()" in code)
...
# ----------------------------------------------------------
print("\n[12] Query Rules（仅非 None 字段提交）")
check("list_tasks nz customer_id",
      "if customer_id is not None:" in source)
check("list_tasks nz customer_id",
      "if customer_id is not None:" in source)
check("list_tasks nz machine_model",
      "if machine_model is not None:" in source)
check("list_tasks default page=1",
      "page: int = 1" in source)
check("list_tasks default page_size=20",
      "page_size: int = 20" in source)
# [13] Body Rules（仅提交非 None 字段）
# ----------------------------------------------------------
check("export_excel body file_name",
      "file_name" in source)
check("export_excel nz customer_id",
      "if customer_id is not None:" in source)
check("export_excel nz process_status",
      "if process_status is not None:" in source)
check("export_excel nz operator_id",
...
check("no logger.info", "logger.info" not in source)
check("no print()", "print(" not in code)
# ----------------------------------------------------------
print("\n[21] Frozen API")
check("no  Server 模块导入", "from server.services" not in source)
check("no  Router 导入", "from server.routers" not in source)
check("no  ORM 模型导入", "from server.models import" not in source)
