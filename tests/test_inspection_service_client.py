"""Test: Desktop InspectionService (Sprint 8 — Task 8.4)

严格依据 DEVELOPMENT_ROADMAP.md Task 8.4 验收标准。
测试 client/services/inspection_service.py 全部公开 API 与代码规范。
...
print("=" * 60)
print("  Task 8.4 — Desktop InspectionService Self Test")
print("=" * 60)

SOURCE_PATH = os.path.join("client", "services", "inspection_service.py")
...
check("InspectionService 有 __init__", hasattr(InspectionService, "__init__"))
...
# ----------------------------------------------------------
print("\n[7] HTTP Mapping")
check("list_inspections → GET /api/inspection",
      '/api/inspection"' in code or "/api/inspection'" in code)
check("get_inspection → GET /api/inspection/{id}",
      '/api/inspection/{' in code)
check("create_inspection → POST /api/inspection",
      'post("/api/inspection"' in code)
check("finish_inspection → POST /api/inspection/{id}/finish",
      '/finish"' in code)
check("update_inspection → PUT /api/inspection/{id}",
      'put(' in code)
check("delete_inspection → DELETE /api/inspection/{id}",
      'delete(' in code)
...
# ----------------------------------------------------------
print("\n[8] 返回 resp.json()")
check("list_inspections 返回 resp.json()", "resp.json()" in code)
check("get_inspection 返回 resp.json()", "resp.json()" in code)
check("create_inspection 返回 resp.json()", "resp.json()" in code)
check("finish_inspection 返回 resp.json()", "resp.json()" in code)
check("update_inspection 返回 resp.json()", "resp.json()" in code)
check("delete_inspection 返回 resp.json()", "resp.json()" in code)
...
# ----------------------------------------------------------
print("\n[10] 零业务逻辑")
check("零 Workflow", "process_status" not in code)
check("零 Status Machine", "result_status" not in code)
...
# ----------------------------------------------------------
print("\n[12] Query Rules（仅非 None 字段提交）")
check("list_inspections 仅提交非 None task_id",
      "if task_id is not None:" in source)
check("list_inspections 仅提交非 None inspector_id",
      "if inspector_id is not None:" in source)
check("list_inspections 仅提交非 None result",
      "if result is not None:" in source)
check("list_inspections 默认 page=1",
      "page: int = 1" in source)
check("list_inspections 默认 page_size=20",
...
# ----------------------------------------------------------
print("\n[13] Body Rules（仅提交非 None 字段）")
check("create_inspection 仅提交非 None 字段",
      "if report_path is not None:" in source)
check("update_inspection body 为空 dict",
      "body: dict[str, Any] = {}" in source)
check("finish_inspection 仅提交非 None failure_reason",
...
# ----------------------------------------------------------
print("\n[16] 循环导入")
check("无循环导入", "from client.services.inspection_service" not in code)
...
# ----------------------------------------------------------
print("\n[18] __all__")
check("__all__ 含 InspectionService", "InspectionService" in source.split("__all__")[-1])
...
check("__init__.py 导出 InspectionService", "InspectionService" in init_source)
